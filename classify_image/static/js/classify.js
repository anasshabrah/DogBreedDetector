$(document).ready(function () {
        ...
        if (!isLoading) {
            ...
            if (e.originalEvent) {
                ...
                reader.onload = function (e) {
                    ...
                    $("#predic").html("<h2>Fetch Dog Breed Now</h2>");
                    $('#predic').show();
                    $('.editable').show();
                    $("#NameDog").text('my dog');
                    $("#social-media").hide();
    ...
    $(".sharer").click(function (event) {
        var url = "https://animal.direct/dog-scanner.html";

        var breedText = $('.rating-process div h1').text();
        var breedName = $('.rating-right-part h3').text();
        if(!breedName){
            breedName = $('.rating-right-part p').first().text();
        }
        var dogNameText = $('#mydog').val();
        var purity =  $('#purity').val();

        var title = dogNameText + " is " + breedText + " "+ breedName+". You can test your dog breed also";

        //facebook
        $('#share-fb').attr('data-url',url).attr('data-sharer', 'facebook');
        //twitter
        $('#share-tw').attr('data-url',url).attr('data-title', title).attr('data-sharer', 'twitter');
        $('#share-tw').removeAttr('data-via');
        //linkedin
